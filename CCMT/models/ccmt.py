...
class CascadedCrossModalTransformer(nn.Module):
    def __init__(self, num_classes, num_patches, dim, depth, heads, mlp_dim, dim_head=64, dropout=0.20):
        super().__init__()
        assert num_patches % 3 == 0, "The number of patches must be equal for all modalities!"
    ...
    def forward(self, x):
        # Split input into three modalities
        # Assuming order: English text, Vietnamese text, Audio
        text1_tokens = x[:, :self.ppm] + self.pos_embedding_text  # English
        text2_tokens = x[:, self.ppm:2*self.ppm] + self.pos_embedding_text_en  # Vietnamese 
        audio_tokens = x[:, 2*self.ppm:] + self.pos_embedding_audio  # Audio

        # First transformer: combine the two text modalities
        tokens_text_cross = self.cross_tr_language(text1_tokens, text2_tokens)
        
        # Second transformer: combine text features with audio
        tokens_cross = self.cross_tr_speech(tokens_text_cross, audio_tokens)

        # Use the first token for classification (like CLS token)
        x = tokens_cross[:, 0]
        return self.mlp_head(x)
    
    def get_class_token_features(self, x):
        """Get the class token features before final classification"""
        # Split input into three modalities
        ...
        # Return the class token features
        return tokens_cross[:, 0]
