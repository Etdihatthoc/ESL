...
def clean_dataframe(df, remove_low_content=True):
    ...
    - Applies replace_repeats with k=2, str="[REPEAT]"
    - Optionally removes rows with low content using is_low_content
    """
    # print(f"Rows before cleaning: {len(df)}")
    df = df.copy()
    df['text'] = df['text'].apply(lambda t: replace_repeats(t, k=3, tag="[REPEAT]"))
...
class ESLGradingModel(nn.Module):
    ...
    def encode(self, input_ids, attention_mask, question_type, visualize=False):
        ...
        with amp.autocast('cuda', enabled=False):
            ...
            if visualize: 
                pooled = (F.normalize(pooled[0], p=2, dim=-1), pooled[1])
            else:
                pooled = F.normalize(pooled, p=2, dim=-1) # Normalize to emphasize cosine distance semantics
