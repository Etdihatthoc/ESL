...
class ESLDatasetWithAudio(Dataset):
    def __init__(self, dataframe, audio_processor=None, remove_low_content=True, 
        ...
        self.num_chunks = num_chunks
        self.chunk_length_sec = chunk_length_sec
        self.is_train = is_train  # Add training flag
...
class ESLTrainerWithAudio(ESLTrainer):
    ...
    def _prepare_data(self):
        ...
        sampling_alpha = 0.5    
        # Pass is_train=True for training dataset
        train_dataset = ESLDatasetWithAudio(train_df, self.audio_processor, is_train=True)
        train_sampler = InverseScoreSampler(train_dataset, alpha=sampling_alpha)
        self.train_loader = DataLoader(
            train_dataset,
            batch_size=self.batch_size,
        ...
        # Pass is_train=False for validation and test datasets
        self.val_loader = DataLoader(
            ESLDatasetWithAudio(val_df, self.audio_processor, is_train=False),
            batch_size=self.batch_size,
            collate_fn=collate_fn,
            num_workers=40,  
            pin_memory=True,
        ...
        self.test_loader = DataLoader(
            ESLDatasetWithAudio(test_df, self.audio_processor, is_train=False),
            batch_size=self.batch_size,
            collate_fn=collate_fn,
            num_workers=40,  
            pin_memory=True,
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            if val_mae < best_val_mae:
                ...
                checkpoint_path = "./model/model_with_audio_bestmae_pretrainedAudioEncoder_aug_newarchitect.pth"  # THAY ĐỔI DÒNG NÀY
...
if __name__ == "__main__":
    ...
    #model = ESLGradingModelWithAudio.load("/media/gpus/Data/AES/ESL-Grading/model/model_with_audio_bestmae_pretrainedAudioEncoder.pth").to(device)
    # ====================
    ...
    batch_size = 4 # Reduced due to audio memory requirements
    epochs = 20
    ...
    #trainer.train()
    trainer.test()
    #trainer.model.save("./model/model_with_audio_final_pretrainedAudioEncoder_aug_newarchitect.pth")
    wandb.finish()